...
A same-board CPU datum bounds what that buys at the smallest size: the identical v05 recurrence compiled in binary64 on one Cortex-A53 core solves the H=3,N=1 stream in about 5.9 microseconds and 1.95 microjoules per solve (PMSM paper, A53 recurrence stream, 2026-09-05), against 105 microseconds and 25.9 microjoules host-resident on the v05 bitstream. The persistent round trip projected below (19 to 23 microseconds) therefore still trails one A53 core at H=3 on both latency and energy. The framework kernel's case at tiny sizes is bit-exact fixed-point equivalence and deterministic timing; its latency and energy case begins where the recurrence's serial cost grows with n and m (H=10 and above), which is why the measurement protocol keeps the H=10 and larger synthetic cells as first-class rows rather than treating H=3 as the headline.
...
For each synthetic size (`n=m` in 1, 6, 20, 64, 320, 372, and 1024), the two route boundaries, and PMSM H=3 and H=10, use 20 warmups and at least 1,000 solves per measured cell. Include `N=1` and `N=101`, zero-event fixtures matching the probe, and event-rich fixtures. Randomize cell order. Use `CLOCK_MONOTONIC_RAW`; in one-shot mode retain the probe interval exactly around `run.start()` through `run.wait()` and do not include buffer syncs. The one-shot CONTROL ARM is the tuned host pattern measured on the v05 bitstream by the PMSM paper on 2026-09-05 (persistent `xrt::run` handle reused across launches, polled completion, no per-period telemetry readback), which reaches about 61 microseconds kernel-wall and 72 microseconds complete at H=3,N=1; the mailbox path is judged against that, not against the 90/106 microsecond interrupt-mode figures. Polled completion on the PS costs about 0.1 W of dynamic power on the same measurement, so the persistent host reports its completion-poll policy (spin, yield, or sleep) and the energy protocol is run for the policy actually used. During INA260 batches nobody logs into the board over ssh; each login costs A53 work and was measured to move the timing median and the energy coefficient of variation. In persistent mode record separate medians for payload write, BO sync, doorbell-to-done, device cycle counter, output sync/read, and total round trip. Configure and refresh are reported separately, with refresh amortized over its declared cadence `K`; no overlap is credited because `b,d` are feedback-dependent.
